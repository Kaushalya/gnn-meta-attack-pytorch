...
class MetaGCN(nn.Module):
    ...
    """
    def __init__(self, n_feat, n_classes, dropout=False, ch_list=None, multilabel=False, sparse=False):
        super(MetaGCN, self).__init__()
        ch_list = ch_list or [n_feat, 256, 128]
        self.multilabel = multilabel
        self.dropout = dropout
        self.gconvs = [GraphLinearLayer(ch_list[i], ch_list[i+1], sparse=sparse) for i in range(len(ch_list)-1)]
